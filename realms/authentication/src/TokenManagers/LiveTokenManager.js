...
class LiveTokenManager {
  ...
  async refreshTokens () {
    ...
    if (!rtoken) throw new Error('Cannot refresh without refresh token')
  ...
  async authDeviceCode (deviceCodeCallback) {
    ...
    const codeRequest = {
      method: 'post',
      body: new URLSearchParams({ client_id: this.clientId, cmode: "xbox", response_type: 'device_code', scope: this.scopes }).toString(),
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      credentials: 'include'
    }
      ...
      try {
        ...
        const token = await fetch(Endpoints.LiveTokenRequest + '?client_id=' + this.clientId, verifi)
          .then(res => res.json()).then(res => {
            if (res.error) {
              if (res.error === 'authorization_pending' || res.error === "expired_token") return
